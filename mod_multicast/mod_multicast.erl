...
%% modified by:  dhinesh.ravi@gmail.com
%%%----------------------------------------------------------------------
...
%%--------------------------------------------------------------------
init([LServerS, Opts]) ->
    %%LServiceS = gen_mod:get_opt(host, Opts, erlang:list_to_binary("multicast." ++ LServerS)),
    LServiceS = gen_mod:get_opt_host(LServerS, Opts, <<"multicast.@HOST@">>),
    Access = gen_mod:get_opt(access, Opts, fun(A) -> A end, all),
    SLimits = build_service_limit_record(gen_mod:get_opt(limits, Opts, fun(A) -> A end, [])),
    create_cache(),
    try_start_loop(),
    create_pool(),
...
handle_info({route, From, To, #xmlel{name = <<"iq">>, attrs = Attrs, children = _Els} = Packet}, State) ->
...
handle_info({route, From, To, #xmlel{name = Stanza_type, attrs = _Attrs, children = _Els}  = Packet},
	    #state{lservice = LServiceS,
		   lserver = LServerS,
		   access = Access,
		   service_limits = SLimits} = State)
  when (Stanza_type == <<"message">>) or (Stanza_type == <<"presence">>) ->
    %io:format("Multicast packet: ~nFrom: ~p~nTo: ~p~nPacket: ~p~naccess: ~p~nservice: ~p~n", [From, To, Packet, Access, SLimits]),
...
process_iq(From, #iq{type = get, xmlns = ?NS_DISCO_INFO, lang = Lang} = IQ, State) ->
    IQ#iq{type = result, sub_el = [#xmlel{name = <<"query">>, attrs = [{<<"xmlns">>, ?NS_DISCO_INFO}], children =  iq_disco_info(From, Lang, State)}]};
%	  [{xmlelement, <<"query">>, [{<<"xmlns">>, ?NS_DISCO_INFO}], iq_disco_info(From, Lang, State)}]};

%% disco#items request
process_iq(_, #iq{type = get, xmlns = ?NS_DISCO_ITEMS} = IQ, _) ->
    IQ#iq{type = result, sub_el = [#xmlel{name = <<"query">>, attrs = [{<<"xmlns">>, ?NS_DISCO_ITEMS}], children = []}]};
%	  [{xmlelement, <<"query">>, [{<<"xmlns">>, ?NS_DISCO_ITEMS}], []}]};

%% vCard request
process_iq(_, #iq{type = get, xmlns = ?NS_VCARD, lang = Lang} = IQ, _) ->
    IQ#iq{type = result, sub_el = [#xmlel{name = <<"vCard">>, attrs = [{<<"xmlns">>, ?NS_VCARD}], children =  iq_vcard( Lang)}]};
%	  [{xmlelement, <<"vCard">>, [{<<"xmlns">>, ?NS_VCARD}], iq_vcard(Lang)}]};

%% version request
process_iq(_, #iq{type = get, xmlns = ?NS_VERSION} = IQ, _) ->
    IQ#iq{type = result, sub_el = [#xmlel{name = <<"query">>, attrs = [{<<"xmlns">>, ?NS_VERSION}], children =  iq_version()}]};
...
-define(FEATURE(Feat), #xmlel{name = <<"feature">>, attrs = [{<<"var">>, Feat}], children = []}).

iq_disco_info(From, Lang, State) ->
    [#xmlel{name = <<"identity">>, attrs = [{<<"category">>, <<"service">>},
       {<<"type">>, <<"multicast">>}, {<<"name">>, <<"Multicast">>}], children = []},
       %{<<"type">>, <<"multicast">>},{<<"name">>, <<(translate:translate(Lang, <<"Multicast">>))>>}], children = []},
 %   [{xmlelement, "identity",
 %     [{"category", "service"},
 %      {"type", "multicast"},
...
%			      [{<<"type">>, <<"bcc">>}, {<<"jid">>, DS}], 
%			      []},
		XML = #xmlel{name = <<"address">>, attrs = [{<<"type">>, <<"bcc">>}, {<<"jid">>, DS}], children = []},
		       #dest{jid_string = DS,
			     jid_jid = D,
			     type = <<"bcc">>,
			     full_xml = XML}
...
%	{xmlelement, <<"addresses">>, AAttrs, Addresses} ->
	#xmlel{name = <<"addresses">>, attrs = AAttrs, children = Addresses} ->
...
%		    Packet_stripped = {xmlelement, Name, Attrs, Els_stripped},
		    Packet_stripped = #xmlel{name = Name, attrs = Attrs, children = Els_stripped},
...
%		  {xmlelement, "address", Attrs, _El} ->
		#xmlel{name = <<"address">>, attrs = Attrs, children = _El} -> 
...
report_not_jid(From, Packet, Dests) ->
    ...
    [route_error(From, From, Packet, jid_malformed, 
		 erlang:list_to_binary("This service can not process the address: " ++ D))
     || D <- Dests2].
...
add_delivered(#xmlel{name = Name, attrs = Attrs, children = Els}) ->
...
%		 [{xmlelement, "addresses", AAttrs, ACs} | C]
		[#xmlel{name = "addresses", attrs = AAttrs, children = ACs} | C]
	 end,
...
check_relay_required(RServer, LServerS, Groups) ->
    case string:str(erlang:binary_to_list(RServer), erlang:binary_to_list(LServerS)) > 0 of
	true -> false;
...
send_query_info(RServerS, LServiceS) ->
    ...
    case string:str(erlang:binary_to_list(RServerS), "echo.") of
	1 -> false;
...
%	      [{xmlelement, "query", [{"xmlns", XMLNS}], []}]},
    Packet = #xmlel{name = <<"iq">>, attrs = [{"to", RServerS}, {"type", "get"}], 
		children = [#xmlel{name = <<"query">>, attrs = [{"xmlns", XMLNS}], children = []}]}, 
...
%    {xmlelement, "query", Attrs2, Els2} = xml:get_subtag(Packet, "query"),
    #xmlel{name = <<"query">>, attrs = Attrs2, children = Els2} = xml:get_subtag(Packet, <<"query">>),
...
get_limits_fields(Fields) ->
    {Head, Tail} = lists:partition(
		     fun(Field) -> 
				...
				#xmlel{name = <<"field">>, attrs = Attrs, children = _Els2} ->
      ...
      fun(Value, R) -> 
		  ...
		  %{xmlelement, "field", Attrs, SubEls} -> 
	 	  #xmlel{name = <<"field">>, attrs = Attrs, children = SubEls} -> 
		      ...
		      #xmlel{name = <<"value">>, children = SubElsV} = SubEls,
...
type_of_stanza(#xmlel{name = <<"presence">>}) -> presence.
...
%	 [{xmlelement, "value", [], [{xmlcdata, Val}]}]}).
     #xmlel{name = <<"field">>, attrs = [{<<"var">>, Var}, {<<"type">>, Type}], 
